...
class ScaleMany:
    ...
    def scale(self, INPUT_FILE, OUTPUT_FILE, directory):
        ...
        for humanoid in humanoids:
            transforms = humanoid.findall('.//Transform')
            for transform in transforms:
                scale = transform.get("scale", "1 1 1").split()
                translation = transform.get("translation", "0 0 0").split()
                ...
                for shape in shapes:
                    for coordinate in shape.findall(".//Coordinate"):
                        ...
                        for num_index, num in enumerate(numbers):
                            numbers[num_index] = float(translation[num_index % 3]) + (float(numbers[num_index]) * float(scale[num_index % 3]))
                        point = ['%.4f' % float(n) for n in numbers]
                        print(point)
                        coordinate.set("point", " ".join(point))
                transform.attrib.pop("scale", None)
                transform.attrib.pop("translation", None)
